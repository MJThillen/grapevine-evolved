...
Begin VB.Form frmLocationList 
   Caption         =   "Locations"
   ClientHeight    =   5070
   ClientLeft      =   1875
   ClientTop       =   750
   ClientWidth     =   7605
   Icon            =   "frmLocationList.frx":0000
   KeyPreview      =   -1  'True
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MDIChild        =   -1  'True
   ...
   Begin VB.Frame fraRight 
      ...
      Begin VB.Image imgIcon 
         Height          =   480
         Left            =   607
         Picture         =   "frmLocationList.frx":058A
         Tag             =   "11"
         Top             =   120
         Visible         =   0   'False
         Width           =   480
   ...
   End
End
Attribute VB_Name = "frmLocationList"
...
'
    ...
    Do Until LocationList.Off
        Set LocationCard = LocationList.Item
        Set NewItem = lvwList.ListItems.Add(, "key" & LocationCard.Name, LocationCard.Name, _
                      , LocationCard.IconKey)
        NewItem.ListSubItems.Add , "type", LocationCard.LocType
        NewItem.ListSubItems.Add , "level", "  " & CStr(LocationCard.Level)
        LocationList.MoveNext
    Loop
    
    lblCount.Caption = CStr(LocationList.Count) & " &Locations"
...
'
    ...
    Dim NewLocation As LocationClass
    Dim NewName As String
    Dim I As Integer
    
    Do
        NewName = InputBox("Enter a name for the location:", "New Location")
        NewName = Trim(NewName)
        If NewName = "" Then Exit Do
        LocationList.MoveTo NewName
    Loop Until LocationList.Off
    
    If NewName <> "" Then
    
        Set NewLocation = New LocationClass
...
'
    ...
    If Not (lvwList.SelectedItem Is Nothing) Then
        ...
        LocationList.MoveTo DelName
        If Not LocationList.Off Then
            Answer = ShiftDown
            If Not Answer Then Answer = (MsgBox("This will PERMANENTLY remove " & DelName & _
                    " from the game. Are you sure you want to delete this location?", _
                    vbYesNo + vbQuestion, "Delete Location") = vbYes)
            If Answer Then
            ...
            End If
        Else
            MsgBox "Grapevine can't find this location!  Was it renamed or deleted?", vbExclamation
        End If
    End If
...
'
    ...
    If Not (lvwList.SelectedItem Is Nothing) Then _
        mdiMain.ShowLocation lvwList.SelectedItem.Text
...
'
    ...
    If mdiMain.CheckForChanges(Me, atLocations) Then
        RefreshList
    Else
        lvwList_ItemClick lvwList.SelectedItem
    End If
...
'
    ...
    If Not (Item Is Nothing) Then
        LocationList.MoveTo Item.Text
        If Not LocationList.Off Then
            imgIcon.Picture = mdiMain.imlIcons.ListImages(LocationList.Item.IconKey).Picture
            imgIcon.Visible = True
            lblName.Caption = Item.Text
            lblOwner.Caption = LocationList.Item.Owner
            lblDate.Caption = Format(LocationList.Item.LastModified, "Short Date")
        Else
            MsgBox "Grapevine can't find this location!  Was it renamed or deleted?", vbExclamation
        End If
    Else
